...
export default function Dashboard({ onLocationSelect }: DashboardProps) {
  ...
  return (
    <>
        ...
        <div className="flex flex-col h-full max-w-7xl mx-auto px-4 py-1.5 md:gap-2 md:py-4">
          <div className="shrink-0 flex justify-between items-center h-10 md:h-12 relative">
            <div className="flex items-center gap-3 z-10">
              <div className="w-8 h-8 md:w-9 md:h-9 bg-linear-to-br from-emerald-400 to-emerald-600 rounded-lg flex items-center justify-center shadow-md shadow-emerald-500/20 transition-all">
                <span className="text-white font-black text-sm md:text-base">PS</span>
              </div>
              <span className="hidden lg:block text-base font-black text-gray-900 dark:text-white tracking-tight">Planuj Směny</span>
            </div>

            <div className="hidden md:block z-10">
              <button 
                onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')} 
                className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 text-gray-500 dark:text-gray-400 transition-all cursor-pointer active:scale-90"
                  ...
                  >
                    {resolvedTheme === 'dark' ? (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M3 12h2.25m.386-6.364l-1.591 1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M3 12h2.25m.386-6.364l-1.591-1.591M12 7.5a4.5 4.5 0 110 9 4.5 4.5 0 010-9z" />
                      </svg>
                    ) : (
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
          ...
          {user && (
            <div className="hidden md:flex items-center gap-3 px-1 py-3 border-b border-gray-200 dark:border-white/5 mb-1">
              <div className="w-10 h-10 rounded-full bg-linear-to-br from-emerald-100 to-emerald-200 dark:from-emerald-900/40 dark:to-emerald-800/40 border-2 border-white dark:border-white/10 shadow-sm flex items-center justify-center text-emerald-700 dark:text-emerald-400 text-xs font-black shrink-0">{getInitials(user.first_name + ' ' + user.last_name)}</div>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide py-2 hidden md:flex md:flex-col gap-4">
            <nav className="flex flex-col gap-0.5">
              {navItems.map((item) => (
                <button key={item.name} className={`flex items-center gap-3 w-full p-2 rounded-xl transition-all duration-200 cursor-pointer ${item.name === 'Dashboard' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold' : 'hover:bg-gray-100 dark:hover:bg-white/5 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}>
                  <svg className={`w-4 h-4 ${item.name === 'Dashboard' ? 'text-emerald-500' : 'text-gray-400 dark:text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={item.icon} /></svg>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial="closed" animate="open" exit="closed" variants={menuVariants} className="fixed inset-0 z-[100] md:hidden bg-linear-to-br from-gray-50 via-white to-gray-100 dark:from-gray-950 dark:via-gray-900 dark:to-slate-950 flex flex-col transition-colors duration-500">
            <div className="flex justify-between items-center px-4 py-3 h-12 shrink-0 border-b border-gray-200 dark:border-white/5">
              <div className="w-8 h-8 bg-emerald-500 rounded-lg flex items-center justify-center shadow-lg"><span className="text-white font-black text-sm">PS</span></div>
              <div className="flex items-center gap-2">
                <button onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')} className="p-2 rounded-xl text-gray-600 dark:text-gray-300 cursor-pointer">
                  <AnimatePresence mode="wait" initial={false}>
                ...
                <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 -mr-2 text-gray-900 dark:text-white cursor-pointer"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-8">
              {user && (
                <motion.div variants={itemVariants} className="flex items-center gap-4 mb-8 p-4 bg-white/50 dark:bg-white/5 rounded-2xl border border-white/20 dark:border-white/5 shadow-sm">
                  <div className="w-14 h-14 rounded-full bg-linear-to-br from-emerald-100 to-emerald-200 dark:from-emerald-900/40 dark:to-emerald-800/40 border-2 border-white dark:border-white/10 shadow-md flex items-center justify-center text-emerald-700 dark:text-emerald-400 text-xl font-black shrink-0">{getInitials(user.first_name + ' ' + user.last_name)}</div>
              ...
              <nav className="flex flex-col gap-1">
                {navItems.map((item) => (
                  <motion.button key={item.name} variants={itemVariants} className="flex items-center gap-4 w-full p-4 rounded-2xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors text-gray-700 dark:text-gray-300"><svg className="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={item.icon} /></svg><span className="text-lg font-bold">{item.name}</span></motion.button>
                ))}
              </nav>
            </div>
            <div className="mt-auto p-6 shrink-0 border-t border-gray-200 dark:border-white/5">
              <motion.button variants={itemVariants} onClick={logout} className="flex items-center justify-center gap-3 w-full py-4 text-lg font-bold text-red-600 bg-red-50 dark:bg-red-900/20 rounded-2xl cursor-pointer"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>Logout</motion.button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
