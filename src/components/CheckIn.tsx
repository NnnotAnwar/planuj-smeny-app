  ...
  return (
    <>
      ...
      <div className="mb-8 hidden flex-col items-center md:flex">
        <div className="relative">
          ...
          <button
            ...
            disabled={isDisabled}
            className={`relative overflow-hidden flex w-auto items-center justify-center gap-2 rounded-xl px-8 py-3 font-medium shadow-sm transition-all active:scale-95 ${buttonClasses} ${!isStarting && !isDisabled ? "animate-pulse-scale" : ""}`}
          >
            {/* Контент кнопки (текст и иконка) */}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 dark:border-white/10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl md:hidden transition-all duration-300">
        {!isDisabled && !isStarting && (
          <div className="absolute inset-6 rounded-2xl bg-emerald-300/50 blur-xl animate-pulse opacity-20"></div>
        )}
        <button
          onClick={onStartShiftClick}
          disabled={isDisabled}
          className={`flex w-full items-center justify-center gap-2 rounded-2xl px-8 py-4 text-lg font-bold shadow-lg transition-all active:scale-[0.98] ${buttonClasses} ${!isStarting && !isDisabled ? "" : ""}`}
        >
          <span className="z-10 flex items-center justify-center gap-2">
            {isStarting ? (
              ...
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={2} fill="currentColor" className="h-6 w-6">
                <path fillRule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.348c1.295.712 1.295 2.573 0 3.285L7.28 19.991c-1.25.687-2.779-.217-2.779-1.643V5.653z" clipRule="evenodd" />
              </svg>
            )}
            {isStarting ? 'Starting...' : 'Start Shift'}
