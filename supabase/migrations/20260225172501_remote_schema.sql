...
ALTER TABLE ONLY "public"."shifts"
    ADD CONSTRAINT "shifts_profile_id_fkey" FOREIGN KEY ("profile_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



CREATE POLICY "Enable read access for ALL users on locations" ON "public"."locations" FOR SELECT USING (true);
...
CREATE POLICY "Enable read access for authenticated users on profiles" ON "public"."profiles" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Enable read access for authenticated users on shifts" ON "public"."shifts" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Organizations are viewable by auth users" ON "public"."organizations" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Users can insert their own shifts" ON "public"."shifts" FOR INSERT TO "authenticated" WITH CHECK (("auth"."uid"() = "profile_id"));



CREATE POLICY "Users can update their own shifts" ON "public"."shifts" FOR UPDATE TO "authenticated" USING (("auth"."uid"() = "profile_id"));



CREATE POLICY "View org locations only" ON "public"."locations" FOR SELECT USING (("organization_id" = ( SELECT "profiles"."organization_id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = "auth"."uid"()))));



CREATE POLICY "View org members only" ON "public"."profiles" FOR SELECT USING (("organization_id" = "public"."get_my_org_id"()));
